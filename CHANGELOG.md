...
- `backend/Caddyfile` — reverse proxy listening on `:8080`. Routes
  `/api/*` and `/_/*` to PocketBase at `localhost:8090` (with
  `flush_interval -1` to preserve SSE for realtime), everything else
  to the SvelteKit Node bundle at `localhost:3000`. `auto_https off`
  because Fly terminates TLS at the edge.
- `backend/entrypoint.sh` — shell supervisor that starts PocketBase,
  the SvelteKit Node bundle, and Caddy, with `trap` + `wait -n` so
  any process dying brings the machine down (Fly restarts).
...
- `backend/Dockerfile` rewritten as three stages: `web-builder`
  ...
  Elm `frontend-builder` stage entirely.
- `fly.staging.toml`: `internal_port` `8090` → `8080` (Caddy now
  fronts the container). Added `ORIGIN =
