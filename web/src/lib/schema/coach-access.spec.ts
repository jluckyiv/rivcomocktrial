...
describe("eligibility_list_entries — all rules coach-gated", () => {
	...
	it("list", () => { assertRules(col); expect(col.listRule).toBe(TEAM_COACH); });
	it("view", () => { assertRules(col); expect(col.viewRule).toBe(TEAM_COACH); });
	it("create", () => { assertRules(col); expect(col.createRule).toBe(TEAM_COACH); });
	it("update", () => { assertRules(col); expect(col.updateRule).toBe(TEAM_COACH); });
	it("delete", () => { assertRules(col); expect(col.deleteRule).toBe(TEAM_COACH); });
});
...
describe("eligibility_change_requests — coaches create; admin approves", () => {
	...
	it("view", () => { assertRules(col); expect(col.viewRule).toBe(TEAM_COACH); });
	it("create", () => { assertRules(col); expect(col.createRule).toBe(TEAM_COACH); });
	it("update is admin-only", () => { assertRules(col); expect(col.updateRule).toBeNull(); });
	it("delete is admin-only", () => { assertRules(col); expect(col.deleteRule).toBeNull(); });
});
...
describe("roster_submissions — public read; coach write", () => {
	...
	it("list is public", () => { assertRules(col); expect(col.listRule).toBe(""); });
	it("view is public", () => { assertRules(col); expect(col.viewRule).toBe(""); });
	it("create", () => { assertRules(col); expect(col.createRule).toBe(TEAM_COACH); });
	it("update", () => { assertRules(col); expect(col.updateRule).toBe(TEAM_COACH); });
	it("delete is admin-only", () => { assertRules(col); expect(col.deleteRule).toBeNull(); });
});
...
describe("attorney_tasks — public read; coach write via roster chain", () => {
	...
	it("list is public", () => { assertRules(col); expect(col.listRule).toBe(""); });
	it("view is public", () => { assertRules(col); expect(col.viewRule).toBe(""); });
	it("create", () => { assertRules(col); expect(col.createRule).toBe(ROSTER_CHAIN); });
	it("update", () => { assertRules(col); expect(col.updateRule).toBe(ROSTER_CHAIN); });
	it("delete", () => { assertRules(col); expect(col.deleteRule).toBe(ROSTER_CHAIN); });
});
...
describe("join_requests — requester or existing coaches read; existing coaches approve", () => {
	...
	it("list", () => { assertRules(col); expect(col.listRule).toBe(JOIN_LIST); });
	it("view", () => { assertRules(col); expect(col.viewRule).toBe(JOIN_LIST); });
	it("create is admin-only (hook creates)", () => { assertRules(col); expect(col.createRule).toBeNull(); });
	it("update (approve/reject)", () => { assertRules(col); expect(col.updateRule).toBe(TEAM_COACH); });
	it("delete is admin-only", () => { assertRules(col); expect(col.deleteRule).toBeNull(); });
});
