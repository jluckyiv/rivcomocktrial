...
import { hasPlayed, sideHistory, toRecords, type MatchHistory, type MatchRecord } from './matchHistory'
...
const winResults = new Map<string, number>([
	// Round 1
	['6,15', 15], ['24,1', 1], ['19,9', 9], ['8,11', 8], ['5,27', 27],
	['26,13', 13], ['10,2', 2], ['3,16', 16], ['4,14', 4], ['28,25', 25],
	['22,17', 22], ['21,12', 12], ['23,20', 20],
	// Round 2
	['9,23', 9], ['2,19', 19], ['27,21', 27], ['16,22', 22], ['13,4', 13],
	['12,6', 12], ['25,3', 25], ['15,5', 5], ['11,24', 11], ['20,26', 20],
	['14,28', 28], ['17,10', 10], ['1,8', 1],
	// Round 3
	['25,1', 1], ['28,5', 5], ['23,17', 23], ['20,12', 12], ['6,3', 6],
	['4,2', 2], ['26,14', 26], ['10,8', 10], ['19,16', 16], ['22,9', 9],
	['21,24', 21], ['15,11', 15], ['27,13', 13],
	// Round 4
	['2,15', 2], ['16,25', 16], ['13,21', 13], ['24,26', 26], ['12,22', 22],
	['11,28', 11], ['5,10', 10], ['1,27', 27], ['3,23', 23], ['8,4', 4],
	['14,6', 14], ['17,19', 19], ['9,20', 9],
])
...
describe('PowerMatch 2026', () => {
	describe('side balance', () => {
		test('every team plays prosecution exactly 2 times', () => {
			...
			const violations = F.allTeams.filter((team) => sideHistory(allHistory, team).prosecution !== 2)
		...
		test('every team plays exactly 4 trials total', () => {
			...
			const violations = F.allTeams.filter((team) => {
				const count = allRecords.filter(
					(r) => r.prosecution === team || r.defense === team
				).length
				return count !== 4
			})
			expect(violations.map((t) => t.number)).toEqual([])
