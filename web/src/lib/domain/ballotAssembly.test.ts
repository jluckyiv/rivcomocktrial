...
describe('BallotAssembly', () => {
	describe('assembleStudent (via assembleScoredPresentation)', () => {
		test('parses first and last name', () => {
			const result = assembleScoredPresentation(fakeScore('Pretrial', 'Prosecution', 'Alice Smith', 7))
	...
	describe('assembleScoredPresentation', () => {
		...
		test('Opening maps to Single weight', () => {
			const result = assembleScoredPresentation(fakeScore('Opening', 'Prosecution', 'Alice Smith', 8))
			expect(result.ok).toBe(true)
		...
		test('CrossExamination is Single weight', () => {
			const result = assembleScoredPresentation(fakeScore('CrossExamination', 'Defense', 'Bob Jones', 5))
			expect(result.ok).toBe(true)
			if (result.ok) expect(result.value.weight).toBe('Single')
		})
		...
		test('points below range (0) returns error', () => {
			const result = assembleScoredPresentation(fakeScore('Opening', 'Prosecution', 'Alice Smith', 0))
			expect(result.ok).toBe(false)
		})

		test('points at minimum (1) succeeds', () => {
			const result = assembleScoredPresentation(fakeScore('Opening', 'Prosecution', 'Alice Smith', 1))
			expect(result.ok).toBe(true)
		})
