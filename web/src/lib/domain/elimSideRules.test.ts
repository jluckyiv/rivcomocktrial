...
describe('ElimSideRules', () => {
	...
	describe('elimSide', () => {
		test('FirstMeeting { Prosecution } → Ok Defense (flip)', () => {
			expect(elimSide({ kind: 'FirstMeeting', mostRecentSide: 'Prosecution' })).toEqual({
				ok: true,
				value: 'Defense'
			});
		});
		...
		test('FirstMeeting { Defense } → Ok Prosecution (flip)', () => {
			expect(elimSide({ kind: 'FirstMeeting', mostRecentSide: 'Defense' })).toEqual({
				ok: true,
				value: 'Prosecution'
			});
		});
	...
	describe('elimSideAssignment', () => {
		test('FirstMeeting { Prosecution } → Ok [Defense, Prosecution]', () => {
			expect(elimSideAssignment({ kind: 'FirstMeeting', mostRecentSide: 'Prosecution' })).toEqual({
				ok: true,
		...
		test('FirstMeeting { Defense } → Ok [Prosecution, Defense]', () => {
			expect(elimSideAssignment({ kind: 'FirstMeeting', mostRecentSide: 'Defense' })).toEqual({
				ok: true,
				value: ['Prosecution', 'Defense']
			});
		});
